    ...
    const getInfo = async () => {
      ...
      if (localStorage.getItem("info")) {
        ...
        const imagesStorage = JSON.parse(
          localStorage.getItem("imageStorageIndex")
        );
        ...
        fetchedData["hydra:member"].forEach((element) => {
          ...
          filesName.then((data) => {
            ...
            localStorage.setItem(
              "imageStorageIndex",
              JSON.stringify(tmpImageStorage)
            );
          });
        });
          ...
          <div className={`index-item${index + 1}`} key={id}>
            <div className={styleByIndex}>
              ...
              <div className="text-center">
                <div>
                  {sortedImages[index] !== undefined && (
                    <img
                      src={imagePath + sortedImages[index]?.name}
                      alt={sortedImages[index]?.name}
